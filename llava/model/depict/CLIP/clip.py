...
def _transform(data_name, size):
    if data_name == "BAPPS":
        assert size == 70, "Image size of BAPPS must be 70"
        return Compose(
            [
                Resize((64, 64), interpolation=NEAREST),
                CenterCrop(size),  # pad 64 to 70
...
def load(
    ...
    if not jit:
        ...
        if str(device) == "cpu":
            model.float()
        return model, _transform("BAPPS", 70), _transform("COCO", size_COCO)
